...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        z = X.dot(W1) + b1
        h = np.maximum(0, z) # Relu operation 
        ...
        # dscores /= N                                    #NxC
        # grads['W2'] = np.dot(h.T,dscores)  + reg * W2          #HxC
        # grads['b2'] = np.sum(dscores,axis=0)
        # dhidden_layer = np.zeros_like(h)
        # dhidden_layer = np.dot(dscores,W2.T)            #NxH
        # dhidden_layer[h <=0] = 0
        # #dhidden_layer = dhidden_layer*(np.maximum(XdotW1,0)>0)
        # grads['W1'] = np.dot(X.T,dhidden_layer)   + reg * W1              #DxH    
